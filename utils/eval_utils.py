...
from model3D import UnetGenerator_3D
...
def save_figs(
    ...
    np.savez_compressed(
        gen_img_path + "losses.npz", d=losses_d, g=losses_g, val=losses_val
    )


def load_g(
    trial, epoch, netG_type, num_downs=7, ngf=64, ups=False, normlayer="instancenorm"
):
    ...
    if "3D" in netG_type:
        net_g = UnetGenerator_3D(
            num_downs=num_downs, ngf=ngf, norm_layer=norm_layer_g_3d, ups=ups
        )
        print(net_g)
    else:
        net_g = UnetGenerator(
    ...
    # load model
    saved_model_path = "{}models/TOF-perf/Trial_{}/epoch{}.pth".format(
        c.root_path, trial, epoch
    )
...
def resize_img(old_img, new_dims):
    if len(old_img.shape) == 4:
        new_img_final = np.empty(
            (new_dims[0], new_dims[1], new_dims[2], old_img.shape[3])
        )
        for i in range(old_img.shape[3]):
            ...
            n_affine = nib.affines.rescale_affine(
                affine=af, shape=old_dims, zooms=zoom, new_shape=new_dims
            )
