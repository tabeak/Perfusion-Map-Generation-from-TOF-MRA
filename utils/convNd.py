...
class ConvNd(nn.Module):
    def __init__(
        ...
        if is_transposed:
            self.conv_f = (nn.ConvTranspose1d, nn.ConvTranspose2d, nn.ConvTranspose3d)[
                max_dims - 1
            ]
        else:
    ...
    def forward(self, input):
        ...
        if not self.is_transposed:
            ...
            if self.rank == 0:
                ...
                if self.padding_mode is "zeros":
                    input = F.pad(
                        input.view(input.shape[0], input.shape[1], input.shape[2], -1),
                        padSize,
                        "constant",
                        0,
                    ).view(inputShape)
        ...
        if not self.is_transposed:
            ...
            size_o = tuple(
                [
                    math.floor(
                        (size_i[x] + 2 * padding[x] - size_k[x]) / self.stride[x] + 1
                    )
            ...
            size_o = tuple(
                [
                    (size_i[x] - 1) * self.stride[x]
                    - 2 * self.padding[x]
                    + (size_k[x] - 1)
                    + 1
                    + self.output_padding[x]
                    for x in range(len(size_i))
                ]
            )
    ...
    def forward(self, x):
        ...
        out = self.norm(
            x.view(
                shape_x[0],
                shape_x[1],
                shape_x[2] * shape_x[3] * shape_x[4] * shape_x[5],
            )
        ).view(shape_x)
        return out


class InstanceNorm4d(nn.Module):
    ...
    def forward(self, X):
        ...
        Y, self.moving_mean, self.moving_var = instance_norm(
            X,
            self.gamma,
            self.beta,
            self.moving_mean,
...
def instance_norm(X, gamma, beta, moving_mean, moving_var, eps, momentum):
    ...
    if not torch.is_grad_enabled():
        ...
        ).detach()  # removed batch dimension for instance norm
        #         var = ((X - mean)**2).mean(dim=(2, 3, 4, 5), keepdim=True).detach()
        var = X.var(dim=(2, 3, 4, 5), unbiased=True, keepdim=True).detach()
        #         assert not torch.any(torch.isnan(mean))
        #         assert not torch.any(var==0)
        # In training mode, the current mean and variance are used for the
        # standardization
        X_hat = (X - mean) / torch.sqrt(var + eps)
        #         print(X_hat[torch.isnan(X_hat)==True])
